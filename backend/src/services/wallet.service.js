...
class WalletService {
    ...
    static async createWallet(userId, keyphrase, locale) {
        ...
        let wallet = new Wallet("polygon-testnet").createWallet(userId, keyphrase)
        ...
        const subject = "AOE Exchange - Wallet Created Sucessfully";
        const body = "Your AOE Exchange wallet has been created sucessfully. Thank you.";
    ...
    static async accessWalletWithPrivateKey(rawData, userId) {
        ...
        var privateKey = rawData.privateKey;
        ...
        privateKey = new Wallet().encrypt(wallet.privatekey); 
    ...
    static async getPriceBNB(rawData) {
        try {
            console.log(rawData);
            const response = await axios.get(`https://api.binance.com/api/v3/ticker/price?symbol=${rawData}`);
            const price = parseFloat(response.data.price);
            console.log(`Price of ${rawData}: ${price} USDT`);
    ...
    static async createMnemonic() {
        try {
            ...
            return {response:true, message:"Success", data:mnemonic}
        } catch (error) {
            console.error('Error fetching price:', error.message);
        } 
    }
    ...
    static async sendToken(rawData, userId) {
        ...
        let user = await UserModel.findOne({id:userId});
        const sendSubject = "AOE Exchange: Tokens sent";
        const sendBody = "You have sent " + rawData.amount + " " + rawData.token + "to " + rawData.destination; 
        ...
        if (receiver) {
            const recvSubject = "AOE Exchange: Tokens received";
            const recvBody = "You have received " + rawData.amount + " " + rawData.token + " tokens in your wallet";
    ...
    static async getCoinForTransaction(rawData, userId) {
        ...
        const sendSubject = "AOE Exchange: Tokens sent";
        const sendBody = "You have sent " + amount + " " + " " + BNB_ADDRESS + " to " + rawData.publicKey;
        emailService.deliverEmail(admin.email, sendSubject, sendBody);

        if (receiver) {
            const recvSubject = "AOE Exchange: Tokens received";
            const recvBody = "You have received " + amount + " " + BNB_ADDRESS + " tokens in your wallet";
            emailService.deliverEmail(receiver.email, recvSubject, recvBody);
        }
        return {response:true, message:"Success", data:null}
    }
